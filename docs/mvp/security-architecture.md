...
5b. 非首次解锁：用主密码解密一条已有密码验证正确性 → PBKDF2 → Ke
       ↓
6. Ke 以不可导出的 CryptoKey 形式仅保存在当前标签页内存中，有效期 10 分钟
       ↓
...
2. 密码列表页面展示（密码明文隐藏为 ••••••••）
       ↓
3. 用户点击"显示"/"复制" → 检查当前标签页内存中是否有有效 Ke
       ↓
4a. 有有效 Ke：直接解密显示
       ↓
...
| 威胁           | 防护                               |
| -------------- | ---------------------------------- |
| XSS 窃取 Ke    | 尽量缩短 Ke 生命周期，仅以内存持有 |
| CSRF           | 依赖 Bearer Token 接口鉴权         |
| 本地存储被读取 | Ke 不进入 storage，刷新/关闭清除   |
